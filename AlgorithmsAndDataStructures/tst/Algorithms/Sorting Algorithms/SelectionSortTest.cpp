...
#include "Algorithms/Sorting Algorithms/SelectionSort.h"
#include <vector>
#include <array>
#include <algorithm>
	...
	void SortAllDistributionVectors(VectorType& distribution_values, SortingType sorting_type) {
		for (auto i = std::size_t(0); i < number_of_generated_vectors_sizes_; ++i)
			for (int j = 0; j < Distributions::number_of_types;++j )
				selection_sorter_.SortContainer(distribution_values[i][j], sorting_type);
	}

	void TearDown() override {}

	SelectionSort selection_sorter_;
...
TEST_F(SelectionSortTest, SortDoubleVector_Ascending) {
...
TEST_F(SelectionSortTest, SortLongVector_Ascending) {
...
TEST_F(SelectionSortTest, SortDoubleVector_Descending) {
...
TEST_F(SelectionSortTest, SortLongVector_Descending) {
