...
#include "Algorithms/Sorting Algorithms/InsertionSort.h"
#include <vector>
#include <array>
#include <algorithm>
	...
	void SortAllDistributionVectors(VectorType& distribution_values, SortingType sorting_type) {
		for (auto i = std::size_t(0); i < number_of_generated_vectors_sizes_; ++i)
			for (int j = 0; j < Distributions::number_of_types; ++j)
				insertion_sorter_.SortContainer(distribution_values[i][j], sorting_type);
	}

	void TearDown() override {}

	InsertionSort insertion_sorter_;
...
TEST_F(InsertionSortTest, SortDoubleVector_Ascending) {
...
TEST_F(InsertionSortTest, SortLongVector_Ascending) {
...
TEST_F(InsertionSortTest, SortDoubleVector_Descending) {
...
TEST_F(InsertionSortTest, SortLongVector_Descending) {
