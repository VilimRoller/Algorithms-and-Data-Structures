	...
	void SortAllDistributionVectors(VectorType& distribution_values, SortingType sorting_type) {
		for (auto i = std::size_t(0); i < number_of_generated_vectors_sizes_; ++i)
			for (int j = 0; j < Distributions::number_of_types; ++j)
				quick_sorter_.SortContainer(distribution_values[i][j], sorting_type);
	}
...
TEST_F(QuickSortTest, SortDoubleVector_Ascending) {
...
TEST_F(QuickSortTest, SortLongVector_Ascending) {
...
TEST_F(QuickSortTest, SortDoubleVector_Descending) {
...
TEST_F(QuickSortTest, SortLongVector_Descending) {
