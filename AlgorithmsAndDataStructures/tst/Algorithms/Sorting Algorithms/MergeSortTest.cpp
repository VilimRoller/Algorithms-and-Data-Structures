...
#include "Algorithms/Sorting Algorithms/MergeSort.h"
#include <vector>
#include <array>
#include <algorithm>
	...
	void SortAllDistributionVectors(VectorType& distribution_values, SortingType sorting_type) {
		for (auto i = std::size_t(0); i < number_of_generated_vectors_sizes_; ++i)
			for (int j = 0; j < Distributions::number_of_types; ++j)
				merge_sorter_.SortContainer(distribution_values[i][j], sorting_type);
	}

	void TearDown() override {}

	MergeSort merge_sorter_;
...
TEST_F(MergeSortTest, SortDoubleVector_Ascending) {
...
TEST_F(MergeSortTest, SortLongVector_Ascending) {
...
TEST_F(MergeSortTest, SortDoubleVector_Descending) {
...
TEST_F(MergeSortTest, SortLongVector_Descending) {
