...
public:
	...
	Container<container_it> SortContainer(const container_it begin_it, const container_it end_it) const noexcept {
		...
		return GetSortedMergedContainer(SortContainer(begin_it, middle_it), SortContainer(middle_it, end_it));
	}
...
private:
	...
	const container_it GetInsertPosition(Container<container_it>& container, const container_value& insert_val, 
	...
	template <class container_it, class container_element>
	void InsertElement(const container_it insert_position_it, container_element& inserting_element) const noexcept {
	...
	template <class container_it>
	container_it GetMedianIterator(const container_it begin_it, int num_of_elements) const noexcept {
	...
	template <class container_it>
	container_it GetRightAdjacentIterator(const container_it iterator) const noexcept {
		return iterator + 1;
	}

	template <class container_it>
	container_it GetLeftAdjacentIterator(const container_it iterator) const noexcept {
		return iterator - 1;
	}
