...
private:
	...
	const int GetDistanceFromBeginIterator(const container_iterator begin_it, const container_iterator end_it,
		const container_value& search_value) const noexcept {
		long long deviation_from_lowest = static_cast<long long>(search_value) - static_cast<long long>(*begin_it);
		long long num_of_elements = static_cast<long long>(end_it - begin_it);
		long long value_range = static_cast<long long>(*(end_it - 1)) - static_cast<long long>(*begin_it);
		if (value_range == long long(0) || deviation_from_lowest == long long(0))
			return 0;
		return (((num_of_elements * deviation_from_lowest) - long long(1)) / value_range);
	}
